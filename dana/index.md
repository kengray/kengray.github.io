...
<section class="hero">
  ...
  <img src="{{ page.bio_photo | relative_url }}" alt="Dana and Ken" class="bio-photo">
  <p class="bio-caption">{{ page.bio_caption }}</p>
  <div class="ctas">
    <a href="#crafts" class="btn solid">See the crafts</a>
    <a href="#music" class="btn line">Meet the band</a>
  </div>
</section>

<div class="zigzag"><svg preserveAspectRatio="none" viewBox="0 0 200 18"><polygon points="0,0 10,18 20,0 30,18 40,0 50,18 60,0 70,18 80,0 90,18 100,0 110,18 120,0 130,18 140,0 150,18 160,0 170,18 180,0 190,18 200,0 200,18 0,18" fill="#221d17"/></svg></div>

<section class="crafts" id="crafts">
  <div class="section-inner">
    <div class="section-head">
      <span class="eyebrow">needle, hook &amp; wheel</span>
      <h2>Quilting, crochet &amp; weaving</h2>
    </div>
    ...
    <div class="craft-group">
      ...
      <div class="patchwork">
        ...
        <div class="patch" style="background-image:url('{{ patch.image | relative_url }}')"><span>{{ patch.caption }}</span></div>
        {% endfor %}
      </div>
      <p>{{ group.text }}</p>
    </div>
    ...
    <div class="craft-group">
      <h3>And the odd side project</h3>
      {% for sp in page.side_projects %}
      <div class="violin-feature"{% unless forloop.first %} style="margin-top: 1.5rem;"{% endunless %}>
        <img src="{{ sp.image | relative_url }}" alt="{{ sp.alt }}">
        <p>{{ sp.text }}</p>
      </div>
      {% endfor %}
    </div>
...
<div class="zigzag flip"><svg preserveAspectRatio="none" viewBox="0 0 200 18"><polygon points="0,0 10,18 20,0 30,18 40,0 50,18 60,0 70,18 80,0 90,18 100,0 110,18 120,0 130,18 140,0 150,18 160,0 170,18 180,0 190,18 200,0 200,18 0,18" fill="#eadfc4"/></svg></div>

<section class="music" id="music">
  <div class="section-inner">
    <span class="eyebrow">stage &amp; strings</span>
    <div class="section-head">
      <h2>{{ page.band_name }}</h2>
    </div>

    <div class="band-poster">
      <img src="{{ page.band_logo | relative_url }}" alt="{{ page.band_name }} logo" style="max-width: 280px; margin: 0 auto 1rem;">
      <h3>{{ page.band_name }}</h3>
      <div class="sub">{{ page.band_sub }}</div>
    </div>

    <img src="{{ page.band_photo | relative_url }}" alt="{{ page.band_photo_alt }}" style="width:100%; border: 3px solid var(--gold); margin-bottom: 1rem;">

    <img src="{{ page.alisa_photo | relative_url }}" alt="{{ page.alisa_photo_alt }}" style="width:100%; border: 3px solid var(--gold); margin-bottom: 0.6rem;">
    <p class="placeholder-note" style="margin-bottom: 2.5rem;">{{ page.alisa_caption }}</p>

    <h3 style="color:var(--gold); font-family:'Fraunces',serif; font-size:1.3rem; margin: 0 0 0.9rem;">Dana's Blue Stage</h3>
    <div class="patchwork" style="grid-template-columns: repeat(3, 1fr);">
      {% for photo in page.blue_stage_photos %}
      <div class="patch" style="background-image:url('{{ photo.image | relative_url }}')"><span>{{ photo.caption }}</span></div>
      {% endfor %}
    </div>
    <img src="{{ page.jam_session_photo | relative_url }}" alt="A full house at Dana's Blue Stage, mid session" style="width:100%; border: 3px solid var(--gold); margin: 1rem 0 0.6rem;">
    <p class="placeholder-note" style="margin-bottom: 2.5rem;">{{ page.jam_session_caption }}</p>

    <div class="band-grid">
      <div>
        <p>{{ page.band_text }}</p>
        <h4 style="font-family:'Fraunces',serif; color: var(--gold); margin: 1.8rem 0 0.4rem;">Line-up</h4>
        <ul class="lineup">
          {% for member in page.lineup %}
          <li><span>{{ member.name }}</span><span>{{ member.role }}</span></li>
          {% endfor %}
        </ul>
      </div>
      ...
      <div>
        <div class="listen-box">
          <h4>Listen</h4>
          <p class="placeholder-note">{{ page.listen_note }}</p>
        </div>
        <div class="shows-box">
          <h4>Upcoming shows</h4>
          <p class="placeholder-note">{{ page.shows_upcoming_note }}</p>
        </div>
        <div class="shows-box">
          <h4>Past shows</h4>
          <ul class="lineup">
            {% for show in page.shows_past %}
            <li><span>{{ show.when }}</span><span>{{ show.what }}</span></li>
            {% endfor %}
          </ul>
        </div>
        <div class="listen-box">
          <h4>Press</h4>
          <p style="font-size:0.9rem;">{{ page.press_text }}</p>
        </div>
      </div>
    </div>

    <h4 style="font-family:'Fraunces',serif; color: var(--gold); margin: 2.5rem 0 0.9rem;">Other projects</h4>
    <div class="band-grid">
      <div>
        <p>{{ page.other_projects_text }}</p>
      </div>
      <div>
        <img src="{{ page.other_projects_image | relative_url }}" alt="{{ page.other_projects_alt }}" style="width:100%; max-width:280px; border: 2px solid var(--ink);">
      </div>
    </div>
  </div>
</section>

<section class="bluegrass" id="bluegrass">
  <div class="section-inner">
    <span class="eyebrow">the festival life</span>
    <div class="section-head">
      <h2>Why bluegrass</h2>
    </div>
    <p>{{ page.bluegrass_text_1 }}</p>
    <div class="quote">{{ page.bluegrass_quote }}</div>
    <p>{{ page.bluegrass_text_2 }}</p>
  </div>
</section>
